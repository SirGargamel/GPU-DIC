...
\item[Scénář] souží pro nastavení přesnosti výpočtu (vyšší přesnost prodlužuje délku výpočtu)
...
\item[SOLVER] [BruteForce | CoarseFine | \textbf{NewtonRaphson}] \\
Řešič použitý pro výpočet. BruteForce je kompletní prohledání všech možností na základě zadaných limitů, jedná se o nejpomalejší a nejnáročnější řešic, zuručuje ale nalezení nejvhodnějšího řešení pro zadané limity. CoarseFine je schopen urychlit výpočet za pomoci postupného zpřesnování řešení na základě hrubého odhadu. Řešič NewtonRaphson je pak implementací Newtonovi metody pro hledání maxima / minima zadané funkce.
...
\end{lstlisting}
\newpage
\section{Ovládání programu DIC\textunderscore Utils}
\label{sec:generator}
\begin{figure}[H]
